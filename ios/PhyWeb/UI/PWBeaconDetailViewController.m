...
@interface PWBeaconDetailViewController () <JBLineChartViewDataSource, JBLineChartViewDelegate, UITableViewDataSource, UITableViewDelegate>
...
- (void)_toggle:(id)sender {
  _showRSSI = !_showRSSI;
  if (_showRSSI) {
    //[_chartView setMaximumValue:256.0];
    [_chartView resetMaximumValue];
    [_chartView resetMinimumValue];
  }
  else {
...
- (void)viewDidLoad {
  ...
  [_chartView setAutoresizingMask:UIViewAutoresizingFlexibleHeight | UIViewAutoresizingFlexibleWidth];
  ...
  [_tableView setAutoresizingMask:UIViewAutoresizingFlexibleTopMargin | UIViewAutoresizingFlexibleHeight | UIViewAutoresizingFlexibleWidth];
  [[self view] addSubview:_tableView];
  
  [self _reloadData];
  
  [self performSelector:@selector(_periodicReloadData) withObject:nil afterDelay:1.0];
}
...
- (void) _reloadData
{
  ...
  for(int i = 0 ; i < _beaconsCount ; i ++) {
    ...
    PWBeacon * beacon = [[[PWBeaconManager sharedManager] beacons] objectAtIndex:i - 2];
    for(NSArray * value in [beacon rssiHistory]) {
      ...
      _rssiValues[i][time] = [nbValue intValue];
      _distanceValues[i][time] = distanceFromRSSI([[beacon uriBeacon] txPowerLevel], [nbValue intValue]);
    }
  }
  for(int i = 0 ; i < _beaconsCount ; i ++) {
...
- (UIColor *)lineChartView:(JBLineChartView *)lineChartView colorForLineAtLineIndex:(NSUInteger)lineIndex
{
  ...
  if (colors == nil) {
    colors = @[[UIColor brownColor], [UIColor blueColor], [UIColor redColor], [UIColor greenColor], [UIColor grayColor], [UIColor orangeColor], [UIColor purpleColor]];
  }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
  ...
  UITableViewCell * cell = [_tableView dequeueReusableCellWithIdentifier:@"Beacon"];
  if (cell == nil) {
    cell = [[UITableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"Beacon"];
  }
  if (_selectedHorizontalIndex == -1) {
    [[cell textLabel] setText:[NSString stringWithFormat:@"%@", [_urls objectAtIndex:row]]];
  }
  else {
    if (_showRSSI) {
      [[cell textLabel] setText:[NSString stringWithFormat:@"%@: %g", [_urls objectAtIndex:row],
