...
public class UrlDeviceTest {
  ...
  private JSONObject mSerializedUrlDevice1;

  @Before
  public void setUp() {
    mUrlDevice1 = new UrlDevice(ID1, URL1);
    mSerializedUrlDevice1 = new JSONObject("{"
        + "    \"id\": \"" + ID1 + "\","
        + "    \"url\": \"" + URL1 + "\""
        + "}");
  }
  ...
  public void jsonSerializeWorks() {
    JSONAssert.assertEquals(mSerializedUrlDevice1, mUrlDevice1.jsonSerialize(), true);
  }

  @Test
  public void jsonDeserializeWorks() {
    assertEquals(mUrlDevice1, mUrlDevice1.jsonDeserialize(mSerializedUrlDevice1));
  }
